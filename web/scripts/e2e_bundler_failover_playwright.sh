...
OUTPUT_DIR="${E2E_OUTPUT_DIR:-$REPO_ROOT/output/playwright/web-bundler-failover}"
mkdir -p "$OUTPUT_DIR"
cd "$OUTPUT_DIR"
...
cleanup() {
  ...
  if [[ -n "${spawned_fail_id:-}" ]]; then
    curl -fsS -X POST "$MONITOR_URL/api/admin/bundlers/${spawned_fail_id}/unregister" \
      -H "authorization: Bearer $adminToken" >/dev/null 2>&1 || true
  fi
  if [[ -n "${spawned_ok_id:-}" ]]; then
    curl -fsS -X POST "$MONITOR_URL/api/admin/bundlers/${spawned_ok_id}/unregister" \
      -H "authorization: Bearer $adminToken" >/dev/null 2>&1 || true
  fi
...
if [[ "$spawn_bundlers" == "1" ]]; then
  ...
  bundlersJson="$(curl -fsS "$MONITOR_URL/api/public/bundlers")"
  upCount="$(
    echo "$bundlersJson" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const j=JSON.parse(d);const up=(Array.isArray(j)?j:[]).filter(b=>b&&b.status==='UP');console.log(up.length);});"
  )"

  if [[ "${upCount:-0}" -lt 1 ]]; then
    echo "No UP bundlers found; spawning a healthy bundler (bundler1) via monitor admin API..."
    okJson="$(
      curl -fsS -X POST "$MONITOR_URL/api/admin/bundlers/spawn" \
        -H "authorization: Bearer $adminToken" \
        -H "content-type: application/json" \
        -d '{"base":"bundler1","name":"bundler1 (e2e failover)"}'
    )"
    spawned_ok_id="$(
      echo "$okJson" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const j=JSON.parse(d); console.log(j.id||'');});"
    )"
  fi

  echo "Spawning a failing bundler (bundler2, failureRate=1) via monitor admin API..."
  failJson="$(
    curl -fsS -X POST "$MONITOR_URL/api/admin/bundlers/spawn" \
      -H "authorization: Bearer $adminToken" \
      -H "content-type: application/json" \
      -d '{"base":"bundler2","name":"bundler2 (e2e failover injected failure)","policy":{"failureRate":1}}'
  )"
  spawned_fail_id="$(
    echo "$failJson" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const j=JSON.parse(d); console.log(j.id||'');});"
  )"
  if [[ -z "${spawned_fail_id:-}" ]]; then
    echo "Failed to parse spawned failing bundler id." >&2
    exit 1
  fi

  echo "Waiting for failing bundler to be UP and for at least one other UP bundler..."
  start="$(date +%s)"
  while true; do
    statusJson="$(curl -fsS "$MONITOR_URL/api/public/bundlers")"
    ok="$(
      echo "$statusJson" | node -e "
        let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{
          const j=JSON.parse(d); const arr=Array.isArray(j)?j:[];
          const failId=process.argv[1];
          const failUp=arr.some(b=>b&&b.id===failId&&b.status==='UP');
          const otherUp=arr.some(b=>b&&b.id!==failId&&b.status==='UP');
          console.log(failUp && otherUp ? '1' : '0');
        });
      " "$spawned_fail_id"
    )"
    if [[ "$ok" == "1" ]]; then
...
FAIL_ID="${E2E_FAIL_BUNDLER_ID:-$spawned_fail_id}"
if [[ -z "${FAIL_ID:-}" ]]; then
  echo "Missing failing bundler id. Either leave E2E_SPAWN_BUNDLERS=1 (default) or set E2E_FAIL_BUNDLER_ID." >&2
  exit 1
fi
FAIL_ID_JS="$(node -e 'console.log(JSON.stringify(process.argv[1] || ""))' "$FAIL_ID")"

pwcli_checked run-code "
  async () => {
  const failId = $FAIL_ID_JS;
  if (!failId) throw new Error('Missing FAIL bundler id.');
  ...
  // Assert failover happened (selected bundler changed away from the failing one).
  await page.waitForFunction((id) => document.querySelector('select')?.value !== id, failId, { timeout: 60_000 });
